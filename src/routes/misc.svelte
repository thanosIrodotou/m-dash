...
<script context="module">
  export async function preload(page, session) {
    const res = await this.fetch('http://localhost:4000/providers/daysOnEarth');
    const daysOnEarth = await res.json();
    return {daysOnEarth};
  }
</script>
...
<script>
  ...
  export let daysOnEarth;
  let items = [];

  function generateLayout(col) {
    return map(daysOnEarth, function (e, i) {
